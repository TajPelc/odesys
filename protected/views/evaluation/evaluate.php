...
<?php if(count($Project->alternatives) > 1 && count($Project->criteria) > 1) { ?>
    <?php echo CHtml::link((($sortType == 'criteria') ? 'Group by alternatives' : 'Group by criteria'), array('evaluation/evaluate'), array('class' => 'button', 'id' => 'sort-type')); ?>
    <?php echo $this->renderPartial('_form', array('eval' => $eval, 'sortType' => $sortType)); ?>
    <?php echo CHtml::link('Continue', array('evaluation/evaluate'), array('class' => 'button right')); ?>
<?php } else { ?>
